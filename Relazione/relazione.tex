...
L'interfaccia del sito si pone l'obiettivo di esporre in modo chiaro ed accessibile quelli che sono i servizi del centro estetico, dando anche l'opportunità di mettersi in contatto con il personale. \\
Inoltre, ai clienti registrati si vuole dare la possibilità di richiedere una prenotazione tramite il sito stesso, attraverso la pagina "Prenotazioni". \\
Per quanto riguarda il lato amministrativo dell'attività, il sito vuole permettere una facile gestione delle richieste di prenotazione attraverso un'apposita dashboard. \\

	\begin{itemize}
		\item \textbf{Accessibilità:} si è deciso di assegnare uno stile comune a tutte le pagine in modo da facilitare l'orientamento e minimizzare il sovraccarico cognitivo durante la navigazione del sito. \\
			\begin{itemize}
				\item %opacità scritte sulle carte, in modo da essere leggibili da screen-reader
				%i contrasti sono a norma almeno AA (speriamo)
				%torna su nel bottone
				%pulsante login per mobile							
				%parlare delle tabelle
			\end{itemize}
...
Di seguito due  esempi di stampa della stessa pagina:\\
	\fbox{\includegraphics[width=0.5\textwidth]{./graphics/NirvanaTrattamentiViso-w.pdf}}
	\fbox{\includegraphics[width=0.5\textwidth]{./graphics/NirvanaTrattamentiViso-wo.pdf}}
